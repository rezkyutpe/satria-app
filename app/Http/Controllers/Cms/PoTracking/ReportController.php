...
use App\Exports\POTracking\VnDataDownload;
...
class ReportController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            if ($this->PermissionMenu('report-mt-fv') == 0  && $this->PermissionMenu('report-pr-po') == 0  && $this->PermissionMenu('report-vn-fv') == 0  && $this->PermissionMenu('report-po-cancel') == 0  && $this->PermissionMenu('report-delivery-gr') == 0  && $this->PermissionMenu('report-kunjungan-qc') == 0 && $this->PermissionMenu('jadwalpengiriman') == 0 && $this->PermissionMenu('report-dl-gd') == 0) {
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-pr-po')->r == 1) {
                $date   = Carbon::now();
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-vn-fv')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
                ...
                $action_menu = $this->PermissionActionMenu('report-vn-fv');
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-mt-fv')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
                ...
                $cekdata = UserVendor::where('Name', Auth::user()->name)->count();
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-po-cancel')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-delivery-gr')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
                ...
                if ($request->action == 1) {
                    ...
                    if ($cekdata > 0) {
                        ...
                        if ($request->month != null  && $request->statusdelv != null && $request->ponumber != null && $request->years != null) {
                            ...
                            $getData = $Data->whereMonth($date, $request->month)->whereYear($date, $request->years)->where('VendorCode', Auth::user()->email)->get();
                        } elseif ($request->statusdelv != null) {
                            $getData = $Data->where('StatusDelivery', $statusdev)->where('StatusReceive', $statusrec)->where('VendorCode', Auth::user()->email)->get();
                        } elseif ($request->years != null) {
                            $getData = $Data->whereYear($date, $request->years)->where('VendorCode', Auth::user()->email)->groupBy('Number', 'ItemNumber')
                                ->orderBy('GoodsReceiptDate', 'DESC')->get();
                        } elseif ($request->ponumber != null) {
                            $getData = $Data->whereIn('Number', $request->ponumber)->where('VendorCode', Auth::user()->email)->get();
                        } else {
                            return redirect()->back()->with('err_message', 'Please Select Item!');
                        }
                        $ponumber = $Data->select('Number')->distinct()->where('VendorCode', Auth::user()->email)->get();
                    } else {
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-kunjungan-qc')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
        ...
        try {
            ...
            if ($this->PermissionActionMenu('jadwalpengiriman')->r == 1) {
                ...
                if (!empty($request->datefilter)) {
                    ...
                    if ((!empty($request->datefilter))) {
                        $datapengiriman = DetailTicket::whereIn('status', $kodex)->where('DeliveryDate', '>=', $date)->whereBetween("DeliveryDate", [$awal, $akhir])->groupBy('Number', 'ItemNumber', 'TicketID')->orderBy('DeliveryDate', 'asc')->get();
                        $datahistorypengiriman = DetailTicket::whereIn('status', $kodex)->where('DeliveryDate', '<=', $date)->whereBetween("DeliveryDate", [$awal, $akhir])->groupBy('Number', 'ItemNumber', 'TicketID')->orderBy('DeliveryDate', 'asc')->get();
                    } else {
                        return redirect()->back()->with('err_message', 'Please Select Item!');
                    }
                } else {
                    $datapengiriman = DetailTicket::whereIn('status', $kodex)->groupBy('Number', 'ItemNumber', 'TicketID')->orderBy('DeliveryDate', 'asc')->get();
                    $datahistorypengiriman = DetailTicket::where('status', 'S')->groupBy('Number', 'ItemNumber', 'TicketID')->orderBy('DeliveryDate', 'asc')->get();
                }
                // dd($getData->toArray());
    ...
    {
        try {
            if ($this->PermissionActionMenu('report-dl-gd')->r == 1) {
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                        'user'  => Auth::user()->email,
    ...
    public function reportDataVendor(Request $request)
    {
        try {
            if ($this->PermissionActionMenu('kelengkapan-data-vendor')->r == 1) {
             
                $getData = VwDataVendor::query();
                $type = $request->searchbytype;

                switch($type){
                    case 'Vendor Local':
                        $getData->where('VendorType','Vendor Local');
                        break;
                    case 'Vendor Import':
                        $getData->where('VendorType','Vendor Import');
                        break;
                    case 'Vendor SubCont':
                        $getData->where('VendorType','Vendor SubCont');
                        break;
    
                }
                $vendor = $getData->get();
                $action_menu = $this->PermissionActionMenu('kelengkapan-data-vendor');
                return view('po-tracking/master/ReportDataVendor', compact('vendor', 'action_menu'));
    ...
    public function vndatadownload()
    {
        try {
            return Excel::download(new VnDataDownload, 'DataVendor.xlsx');
        } catch (Exception $e) {
            $this->ErrorLog($e);
            return redirect()->back()->with('error', 'Error Request, Exception Error ');
        }
