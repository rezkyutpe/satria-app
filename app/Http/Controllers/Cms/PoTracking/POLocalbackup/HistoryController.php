...
class HistoryController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            ...
            if ($this->PermissionMenu('polocalhistory') == 0 && $this->PermissionMenu('polocalhistory-nonmanagement') == 0 && $this->PermissionMenu('polocalhistory-whs') == 0 && $this->PermissionMenu('polocalhistory-proc') == 0 && $this->PermissionMenu('polocalhistory-vendor') == 0) {
    ...
    public function Polocalhistory()
    {
        ...
        if($this->PermissionActionMenu('polocalhistory')->r==1 ){
            ...
            $data = VwHistoryLocal::select('ID','Number','NRP','PurchaseOrderCreator','Vendor','VendorCode','VendorCode_new','POID','Date','ReleaseDate','DeliveryDate','ParentID','ItemNumber',
            'Material','MaterialVendor','Description','NetPrice','Currency','Quantity',
           'ConfirmedDate','ConfirmedQuantity','IsClosed','ActiveStage','totalgr')->
          ...
          if($this->PermissionActionMenu('polocalhistory-nonmanagement')->r==1 ){
            ...
            ['time'     => $date->toTimeString()
             ...
             $data = VwHistoryLocal::select('ID','Number','NRP','PurchaseOrderCreator','Vendor','VendorCode','VendorCode_new','POID','Date','ReleaseDate','DeliveryDate','ParentID','ItemNumber',
              ...
              return view('po-tracking/polocal/pohistory-nonmanagement',
              compact(
                  'data',
                  'header_title',
                  'link_newPO',
   ...
   public function PolocalhistoryVendor()
   {
    ...
    public function PolocalhistoryWhs()
    {
        if($this->PermissionActionMenu('polocalhistory-whs')->r==1 ){
            ...
            $planDelivery = DetailTicket::select('Number','ItemNumber','TicketID')->where('status','P')->groupBy('Number', 'ItemNumber','TicketID')->orderBy('ItemNumber', 'asc')->get();
            $readyToDelivery = DetailTicket::select('Number','ItemNumber','TicketID')->distinct('Number','ItemNumber','TicketID')->whereIn('status',$kodex)->get();
            $data = VwHistoryLocal::select('ID','Number','NRP','PurchaseOrderCreator','Vendor','VendorCode_new','VendorCode','POID','Date','ReleaseDate','DeliveryDate','ParentID','ItemNumber',
                 'Material','MaterialVendor','Description','Quantity','totalgr',
                'ConfirmedDate','ConfirmedQuantity','ActiveStage','IsClosed')->
                 groupBy('Number', 'ItemNumber')->orderBy('id', 'desc')->get();
