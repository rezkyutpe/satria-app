...
class NewPoController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            if ($this->PermissionMenu('subcontractornewpo') == 0 && $this->PermissionMenu('subcontractornewpo-management') == 0 && $this->PermissionMenu('subcontractornewpo-nonmanagement') == 0 && $this->PermissionMenu('subcontractornewpo-whs') == 0 && $this->PermissionMenu('subcontractornewpo-proc') == 0 && $this->PermissionMenu('subcontractornewpo-vendor') == 0) {
    ...
    public function subcontractornewpo()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo')->r == 1) {
                ...
                $number_potracking = VwSubcontNewpo::select('Number')->distinct()->get()->toArray();
                ...
                return view(
                    'po-tracking/subcontractor/subcontractornewpo',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function subcontractornewpoManagement()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo-management')->r == 1) {
                $header_title                   = "PO SUBCONT - NEW PO";
                $date   = Carbon::now();
                ...
                $kodex = ['A', 'D'];
                $actionmenu =  $this->PermissionActionMenu('subcontractornewpo-management');
                $data = VwSubcontNewpo::select(
                    'Number',
                    'NRP',
                    'PurchaseOrderCreator',
                ...
                $OngoingSubcont  = VwSubcontOngoing::select('POID', 'ItemNumber')->whereIn('Plant', $plant)->distinct('POID', 'ItemNumber')->count();
    ...
    public function subcontractornewpoNonManagement()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo-nonmanagement')->r == 1) {
                ...
                $countReadyToDeliverySubcont   = $readyToDelivery;
                return view(
                    'po-tracking/subcontractor/subcontractornewpononmanagement',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function subcontractornewpoProc()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo-proc')->r == 1) {
                ...
                $countReadyToDeliverySubcont   = $readyToDelivery;
                return view(
                    'po-tracking/subcontractor/subcontractornewpoproc',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function subcontractornewpoVendor()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo-vendor')->r == 1) {
                ...
                $countReadyToDeliverySubcont   = $readyToDelivery;
                return view(
                    'po-tracking/subcontractor/subcontractornewpovendor',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function subcontractornewpoWhs()
    {
        try {
            if ($this->PermissionActionMenu('subcontractornewpo-whs')->r == 1) {
                ...
                // $data1 = VwSubcontNewpo::select(
                //     'Plant'
                // )->whereIn('Plant', $plant)->distinct()->get();
                // dd($data1);
                $data = VwSubcontNewpo::select(
                    'Number',
                    'NRP',
                ...
                $planDelivery = DetailTicket::select('Number', 'ItemNumber', 'TicketID')->whereIn('Plant', $plant)->distinct('Number', 'ItemNumber', 'TicketID')->where('status', 'P')->count();
                $HistorySubcont = VwSubcontHistory::select('Number', 'ItemNumber')->whereIn('Plant', $plant)->distinct('Number', 'ItemNumber')->count();
                $readyToDelivery = DetailTicket::select('Number', 'ItemNumber', 'TicketID')->distinct('Number', 'ItemNumber', 'TicketID')->whereIn('Plant', $plant)->whereIn('status', $kodex)->count();
                $material_potracking = VwSubcontNewpo::select('Material')->distinct()->get()->toArray();
                $ccr_reqdate = VwPoTrackingReqDateMaterial::whereIn('material', $material_potracking)->groupby('material')->selectRaw('material,MIN(req_date) AS req_date')->get();
                foreach ($data as $a) {
                    foreach ($ccr_reqdate as $b) {
