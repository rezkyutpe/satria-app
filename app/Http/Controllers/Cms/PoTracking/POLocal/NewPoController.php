...
class NewPoController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            if ($this->PermissionMenu('polocalnewpo') == 0 && $this->PermissionMenu('polocalnewpo-management') == 0 && $this->PermissionMenu('polocalnewpo-nonmanagement') == 0 && $this->PermissionMenu('polocalnewpo-proc') == 0 && $this->PermissionMenu('polocalnewpo-whs') == 0 && $this->PermissionMenu('polocalnewpo-vendor') == 0) {
    ...
    public function polocalnewpo()
    {
        try {
            if ($this->PermissionActionMenu('polocalnewpo')->r == 1) {
                ...
                $number_potracking = VwLocalnewpo::select('Number')->distinct()->get()->toArray();
                ...
                return view(
                    'po-tracking/polocal/polocalnewpo',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    //Po Local New PO View Management
    public function polocalnewpoManagement()
    {
        try {
            if ($this->PermissionActionMenu('polocalnewpo-management')->r == 1) {
                $header_title                   = "PO LOCAL - New PO";
                $date   = Carbon::now();
                LogHistory::updateOrCreate(
                    [
                ...
                $kodex = ['A', 'D'];
                $actionmenu =  $this->PermissionActionMenu('polocalnewpo-management');
                ...
                $OngoingPolocal  = VwOngoinglocal::select('POID', 'ItemNumber')->whereIn('Plant', $plant)->distinct('POID', 'ItemNumber')->count();
    ...
    public function polocalnewpoNonManagement()
    {
        try {
            if ($this->PermissionActionMenu('polocalnewpo-nonmanagement')->r == 1) {
                ...
                $countreadyToDelivery   = $readyToDelivery;
                return view(
                    'po-tracking/polocal/polocalnewpononmanagement',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function polocalnewpoProc(Request $request)
    {
        try {
            if ($this->PermissionActionMenu('polocalnewpo-proc')->r == 1) {
                ...
                $countreadyToDelivery   = $readyToDelivery;
                return view(
                    'po-tracking/polocal/polocalnewpoproc',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
    ...
    public function polocalnewpoVendor()
    {
        try {
            if ($this->PermissionActionMenu('polocalnewpo-vendor')->r == 1) {
                ...
                $countreadyToDelivery   = $readyToDelivery;
                return view(
                    'po-tracking/polocal/polocalnewpovendor',
                    compact(
                        'data',
                        'header_title',
                        'link_newPO',
