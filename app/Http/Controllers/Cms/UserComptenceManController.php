...
use Exception;
class UserComptenceManController extends Controller
{
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            
            if ($this->PermissionMenu('user-competence') == 0){
    ...
    public function UserComptenceMgmtInit(Request $request)
    {
        try{
            if($this->PermissionActionMenu('user-competence')->r==1){
                $response = Http::withHeaders(['Authorization' => env('ENV_TOKEN'),'x-api-type'=>'SKGOL'])->get('http://satria2.patria.co.id/satria-api-man/public/api/performance-list');
                $res = json_decode($response,true);
                $data = array(
                    'pis' => $res['data'],
                    // 'actionmenu' => $this->PermissionActionMenu('pis-management'),
                );
                return view('user-management/user-competence')->with('data', $data);
            }else{
                return redirect()->back()->with('err_message', 'Akses Ditolak!');
            }  
        } catch (Exception $e) {    
    ...
    public function UserComptenceMgmtPreview(Request $request)
    {
        try{
            if($this->PermissionActionMenu('user-competence')->r==1){
                ...
                return view('user-management/raw-preview-competence')->with('data', $data);
            }else{
                return redirect()->back()->with('err_message', 'Akses Ditolak!');
            }  
        } catch (Exception $e) {    
    ...
    public function UserComptenceMgmtInsert(Request $request)
    {
        try{
            if($this->PermissionActionMenu('user-competence')->r==1){
                if($request->input('letterno'))
                {
                    ...
                    return redirect('user-competence')->with('suc_message', 'Data berhasil ditambahkan!');
