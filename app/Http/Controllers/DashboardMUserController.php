...
class DashboardMUserController extends Controller
{
    public function index()
    {
        ...
        $ticketassign = VwTicket::where('dept', Auth::user()->dept)->where('assist_id', Auth::user()->id)->where('created_at', 'like', "%" . date('Y') . "%")->orderBy('created_at', 'desc')->get();
        // dd($ticketassign);
        ...
        // dd($newtrbar3, $barticketresolved, $newtcd);
        ...
        $tickettotalcanceled = VwTicket::select(DB::raw('count(ticket_id) as total_ticket'))->where('dept', Auth::user()->dept)->where('flow_name', 'Canceled')->whereBetween('created_at', [$firsttcd . " 00:00:00", $lasttcd . " 23:59:59"])->get();
        ...
        // dd($ticketreqdate);
        // dd($dialyticket);
        return view('dashboard-maintenance-user', compact('topassist', 'newtr2', 'newtrd2', 'newtto2', 'newtti2', 'newttca2', 'newtte2', 'newttcl2', 'newttc2', 'newttr2', 'newtcbar2', 'newtrbar2', 'newtrclbar2', 'newtcd2', 'ticketassign', 'ticketopened', 'allticketassign', 'topsla', 'topreporter', 'prosesticketbyassist', 'sla', 'assist', 'assist2', 'inventory', 'subject', 'subjectexport'));
        } catch (Exception $e) {
            $this->ErrorLog($e);
    ...
    {
        $ticketassign = VwTicket::select(DB::raw('*,DATE_FORMAT(created_at, "%H:%i:%s, %d %b %Y ") as date'))->where('dept', Auth::user()->dept)->where('assist_id', Auth::user()->id)->whereBetween('created_at', [$request->Start_yta . " 00:00:00", $request->End_yta . " 23:59:59"])->orderBy('created_at', 'desc')->get();
    ...
    public function TicketMgmtNewUser(Request $request)
    {
        try {
            $paginate = 250;
            $ticket = VwTicket::where('dept', Auth::user()->dept)->where('assist_id','=',null)->where('flag', 1)->where('created_at', 'like', "%" . date('Y-m') . "%")->orderBy('created_at', 'desc')->simplePaginate($paginate);
            $sla = MstSla::where('dept', Auth::user()->dept)->orderBy('name', 'asc')->get();
            $assist = UserRoleGroup::select('user_role_group.*', 'users.name')->join('users', 'users.id', '=', 'user_role_group.user')->where('users.dept', Auth::user()->dept)->whereIn('group', [14, 16])->get();
            $inventory = VwInventory::where('dept', Auth::user()->dept)->where('flag', 1)->orderBy('inventory_nama', 'asc')->get();
            $no = 1;
            ...
            return view('elsa-assist/user-all-ticket')->with('data', $data);
        } catch (Exception $e) {
            $this->ErrorLog($e);
            return redirect()->back()->with('err_message', 'Error Request, Exception Error ');
        }
    ...
    public function FilterTicketMgmtNewUser(Request $request)
    {
        try {
            $paginate = 250;
            $ticket = VwTicket::where('dept', Auth::user()->dept)->where('assist_id','=',null)->where('flag', 1)->whereBetween('created_at', [$request->startdate . " 00:00:00", $request->enddate . " 23:59:59"])->orderBy('created_at', 'desc')->simplePaginate($paginate);
            $sla = MstSla::where('dept', Auth::user()->dept)->orderBy('name', 'asc')->get();
            $assist = UserRoleGroup::select('user_role_group.*', 'users.name')->join('users', 'users.id', '=', 'user_role_group.user')->where('users.dept', Auth::user()->dept)->whereIn('group', [14, 16])->get();
            $inventory = VwInventory::where('dept', Auth::user()->dept)->where('flag', 1)->orderBy('inventory_nama', 'asc')->get();
            $no = 1;
    ...
    public function AllTicketMgmtAssistUser(Request $request)
    {
        try {
            $paginate = 250;
            ...
            $data = array(
                'title' => 'List All Ticket Assigned',
                'slug' => 'assign',
                'startdatemonth' => Carbon::now()->startOfMonth()->toDateString(),
                'enddatemonth' => Carbon::now()->endOfMonth()->toDateString(),
                'inventory' => $inventory,
    ...
    public function FilterAllTicketMgmtAssistUser(Request $request)
    {
        try {
            $paginate = 250;
