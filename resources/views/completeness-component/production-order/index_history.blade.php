                                        ...
                                        @foreach ($data['db'] as $item)                                          
                                            <tr>
                                                ...
                                                <td data-order="{{ $item->production_order }}">
                                                    @if ($data['actionmenu']->r == 1)
                                                        <a href="{{ url('production-order-planning-history/' . $item->production_order) }}" class="text-dark">
                                                ...
                                                <td data-toggle="tooltip" data-placement="right" title="{{ $item->product_description }}" data-order="{{ $item->product_number }}">
                                                    @if ($data['actionmenu']->r == 1)
                                                        <a href="{{ url('planning-unit-history/' . $item->product_number) }}" class="text-dark">
                                                            {{ $item->product_number }}
                                                        </a>                                                       
                                                    @else
                                                        {{ $item->product_number }}
                    ...
                    <?php if ($data['actionmenu']->v==1){ ?> 
                        {
                            ...
                            action: function ( e, dt, node, config ) {
                                window.location.href = "{{ url('production-order-history-download') }}";
                            }
                        }
                    <?php } ?>
                ],
