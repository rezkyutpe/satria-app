        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @csrf
                    ...
                    <div class="cols float-left p-2" id="idparkingnumber"></div>
        ...
        $('.parkinginvoice_detail').on('click', function ()
        {
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    // console.log(data.header.ParkingNumber);
                    var parkingnumber = data.header.ParkingNumber == null ? '-' : data.header.ParkingNumber;
                    $("#idparkingnumber").append('Parking Number : <strong>'+parkingnumber+'</strong>');
                    var totalprice = 0;
                    var ppn = 0;
                    var amounttotal = 0;
                    ...
                    for(j=0;j<data.document.length;j++){
                        $("#downloadparking").append(`
                            <a class="btn btn-danger btn-sm" href="{{ url('public/potracking/parking_invoice/`+data.header.Number+`/`+data.document[j].FileName+`') }}" formtarget="_blank" target="_blank"> <i class="fa fa-download"> </i> `+data.document[j].FileName+`</a>
                        `);
                    }
                    ...
                    else if(data.header.Status == "Request Parking" && (data.user == "adm.proc2@patria.co.id" || data.user == "agamad.patria@gmail.com")){
                        var color = "primary";
                        var fungsi = "button";
                        var remark = "Update";
                        var ico = "";
                    ...
                    else if(data.header.Status == "Receive Document" && (data.user == "adm.proc2@patria.co.id" || data.user == "agamad.patria@gmail.com")){
                        var color = "primary";
                        var fungsi = "button";
                        var remark = "Update";
                        var ico = "";
