    ...
    <div class="content flex-row-fluid" id="kt_content">
            <div class="card card-flush">
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    <a href="{{ url('open-ticket') }}" class="btn btn-color-gray-600 btn-active-color-primary"><span class="svg-icon svg-icon-5"><i class="fa fa-angle-left fs-2"></i></span>List Open Ticket</a>
                     
                </div>
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    <h2 class="text-end pe-0">Your Ticket Assigned</h2>

                             <form action="{{ url('assist-ticket') }}" method="get">
                            <input type="month" name="q" id="q" class="form-control form-control-solid" value="{{ isset($_GET['q']) ? $_GET['q'] : date('Y-m') }}" onchange="this.form.submit()">
                               <!--  <select class="form-select form-select-solid"  name="q" id="q" onchange="this.form.submit()">
                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    <div class="card-title">
                        ...
                        <button type="submit" id="multiple-close" style="display: none;" class="btn btn-danger">Close Selected <span id="count_check"></span> </button>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-end min-w-50px">Rate</th>
                        ...
                        @php($duration = '('.floor($datediff/60).':'.gmdate('i:s', $interval*60).')')
                            <tr>
                                ...
                                <div class="form-check form-check-sm form-check-custom form-check-solid">
                                    <input class="form-check-input" id="selected-item" name="ticketselect[]" onchange="selectTicket()" type="checkbox" value="{{ $ticket->id }}" @if($ticket->flag!=3) disabled @endif  />
                                ...
                                <td class="pe-0" data-order="20">
                                    ...
                                    <span style="color: grey;font-size:12px">{{ $ticket->dept_reporter }}</span><br>
                                    <span style="color: grey;font-size:11px">At {{ date('d F Y H:i:s',strtotime($ticket->created_at))}}</span></span>
                                </td>
                                <td class="text-end pe-0" data-order="rating-3">
                                    <div class="d-flex flex-column w-100 me-2 mt-2">
                                                        <span class="text-gray-400 me-2 fw-boldest mb-2">{{ $intervalpercent<=100 ? $intervalpercent : '100+' }}%  {{ isset($ticket->respond_time) ? $duration : '-' }}</span>
                                                        <div class="progress bg-light-danger w-100 h-5px">
                                                            ...
                                                            <span class="text-gray-400 fw-bold d-block">SLA: {{ $ticket->sla_name }}</span>
                                                    </div>
                                </td>
                                ...
                                <td class="text-end pe-0">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                        ...
                                        @if($ticket->flag==1)
                                            <div class="menu-item px-3">
                                                <a href="#" onclick="setProccess({{ $ticket->id }},'Proccess')" class="menu-link px-3" tooltip="New App" data-bs-toggle="modal" data-bs-target="#action-ticket"  class="menu-link px-3">Proccess</a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="#" onclick="setCancel({{ $ticket->id }},'Cancel')" class="menu-link px-3" tooltip="New App" data-bs-toggle="modal" data-bs-target="#action-ticket"  class="menu-link px-3">Cancel</a>
                                            </div>
                                        @elseif($ticket->flag==2 || $ticket->flag==3 ||$ticket->flag==9)
                                           @if($ticket->flag!=3)
                                            <div class="menu-item px-3">
                                                <a href="#" onclick="setEscalate({{ $ticket->id }},'Escalate')" class="menu-link px-3" tooltip="New App" data-bs-toggle="modal" data-bs-target="#action-ticket" class="menu-link px-3">Escalate</a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="#" onclick="setResolve({{ $ticket->id }},'Resolve')" class="menu-link px-3" tooltip="New App" data-bs-toggle="modal" data-bs-target="#action-ticket" class="menu-link px-3">Resolve</a>
                                            </div>
                                            @else
                                                ...
                                                @if($interval>='32400')
                                                <div class="menu-item px-3">
                                                    <a href="#" onclick="setClose({{ $ticket->id }},'Close')" class="menu-link px-3" tooltip="New App" data-bs-toggle="modal" data-bs-target="#action-ticket" class="menu-link px-3">Close</a>
                                                </div>
                                                @endif
                                            @endif
