...
<div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
    ...
    <div class="card mb-5 mb-xl-10">
            ...
            <form method="post" action="{{url('insert-user-performance')}}" enctype="multipart/form-data">
            {{csrf_field()}}
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                            ...
                            <table class="table table-hover table-rounded table-striped border gy-7 gs-7">
                                ...
                                <tbody>
								    @for($i=0;$i < count($data['pis']);$i++)
									    <tr>
                                            <td>
                                                <input type="text" class="form-control" name="letterno[]" value="{{ 'PK.BNK/'.date_format(date_create(str_replace('/','-',$data['pis'][$i][0])),'m/Y').($data['lastrows']+$i+1).'/'.$data['pis'][$i][2] }}">
                                            </td>
                                            <td>
